...
from minigpt4.pruning.prune import prune_wanda, check_sparsity
...
prune_cfg = getattr(cfg.model_cfg, "prune", None)
if prune_cfg and prune_cfg.get("enabled", False):
    llm = model.llama_model
    llm.seqlen = getattr(prune_cfg, "seqlen", None) or llm.config.max_position_embeddings
    prune_args = SimpleNamespace(
        sparsity_ratio=prune_cfg.get("sparsity_ratio", 0.5),
        nsamples=prune_cfg.get("nsamples", 64),
        seed=prune_cfg.get("seed", 0),
        use_variant=False,
    )
    print("Pruning LLM with Wanda...")
    prune_wanda(prune_args, llm, model.llama_tokenizer, device=torch.device("cuda:0"))
    print("Sparsity after pruning:", check_sparsity(llm))
