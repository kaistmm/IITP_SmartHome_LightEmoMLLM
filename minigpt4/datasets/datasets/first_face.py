...
class FeatureFaceDataset(Dataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_path):
        ...
        json_file_path = "eval_configs/MERR_coarse_grained.json" 
        with open(json_file_path, 'r') as json_file:
            self.MERR_coarse_grained_dict = json.load(json_file)

        reason_json_file_path = "eval_configs/MERR_fine_grained.json"
        with open(reason_json_file_path, 'r') as json_file:
            self.MERR_fine_grained_dict = json.load(json_file)

        self.character_lines = pd.read_csv('eval_configs/transcription_en_all.csv')
    ...
    def get(self, video_name):
        # FaceMAE feature
        FaceMAE_feats_path = os.path.join(self.file_path, 'mae_340_UTT_MER2023-SEMI', video_name + '.npy')
        FaceMAE_feats = torch.tensor(np.load(FaceMAE_feats_path))

        # VideoMAE feature
        VideoMAE_feats_path = os.path.join(self.file_path, 'maeV_399_UTT_MER2023-SEMI', video_name + '.npy')
        VideoMAE_feats = torch.tensor(np.load(VideoMAE_feats_path))

        # Audio feature
        Audio_feats_path = os.path.join(self.file_path, 'HL-UTT_MER2023-SEMI', video_name + '.npy')
